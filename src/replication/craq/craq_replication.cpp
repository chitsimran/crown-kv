...
CleanState GetCommittedState(const std::string& key) {
    CleanState state;
...
bool HasDirty(const std::string& key) {
...
std::optional<Record> GetDirtyRecord(const std::string& key, uint64_t version) {
    std::lock_guard<std::mutex> lock(KVStore::mutex_dirty_store_);
    auto it = KVStore::dirty_store_.find(key);
...
PutResponse CraqReplication::handle_put(PutRequest request) {
    ...
    CleanState clean_state = GetCommittedState(request.key());
...
GetResponse CraqReplication::handle_get(std::string key) {
    ...
    if (!HasDirty(key)) {
        ...
        CleanState clean_state = GetCommittedState(key);
    ...
    if (node_index == tail_index) {
        committed_version = GetCommittedState(key).version;
    } else {
    ...
    auto dirty_record = GetDirtyRecord(key, committed_version);
