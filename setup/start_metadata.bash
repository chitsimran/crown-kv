...
build_project() {
    ...
    if [[ ! -x build/metadata_store ]]; then
        echo "ERROR: build/metadata_store not found after build."
        exit 1
    fi
}
...
start_metadata() {
    ...
    mkdir -p "$run_dir"
    local pid_file="$run_dir/metadata_${METADATA_PORT}.pid"
    local log_file="$run_dir/metadata_${METADATA_PORT}.log"
    local out_file="$run_dir/metadata_${METADATA_PORT}.out"
